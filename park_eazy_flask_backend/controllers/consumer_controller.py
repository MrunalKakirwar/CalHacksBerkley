...
#         response = lenders_collection.find().to_list(length=None)
...
# Setup MongoDB connection (make sure MongoDB is running)
# client = MongoClient('mongodb://localhost:27017/')  # Adjust this connection string if needed
# db = client['park_eazy']  # Replace 'park_eazy' with your database name
# lenders_collection = db['lenders']  # Replace 'lenders' with your collection name
...
lenders_collection = db['lenders']  # Replace with your collection name
...
def search_parking_controller():
    ...
    try:
        ...
        # Fetch the lenders from MongoDB collection
        response = lenders_collection.find()  # Retrieve all lenders from the MongoDB collection
